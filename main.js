...
function setup(){
    ...
    Canvas.center();
    background("white");
    Canvas.mouseReleased(rtisan);
    myspeaker=window.speechSynthesis;
}
...
function preload(){
    Store1=ml5.imageClassifier("DoodleNet");
...
function rtisan(){
    Store3.classify(Canvas,getResult);
}
function getResult(error,arraywon){
    if(error){
        console.log(error);

    }
    console.log(arraywon);
    document.getElementById("accuracy").innerHTML=Math.round(arraywon[0].confidence*100)+"%";
    document.getElementById("ursketch").innerHTML=arraywon[0].label;
    Speaker=new SpeechSynthesisUtterance(arraywon[0].label);
    myspeaker.speak(Speaker);
}
